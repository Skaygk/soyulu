...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!buffer || buffer.length === 0) return res.status(400).json({ ok: false, error: 'Archivo no recibido' });
    if (buffer.length > MAX_UPLOAD) return res.status(400).json({ ok: false, error: 'Archivo supera 10MB' });
    ...
    if (!response.ok) return res.status(500).json({ ok: false, error: 'Error al subir a R2' });
